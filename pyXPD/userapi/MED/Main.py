...
def ocilatorgen(functype, cycles, period, phase, resolution):
    """
    Generates an ocilating function for MED experiments
    Parameters
    ----------
    functype: str
...
def MED(cycles=None, period=None,
    ...
    ocil1=ocilatorgen(oscillator, cycles, period, 0)
    ocil2=ocilatorgen(oscillator, cycles, period, 1)

    variableD=dict()
    for key in kwargs.keys():
        if key not in ['Constant', 'Const']:
            variableD[key]=kwargs[key]
            variableD[key]['aplitude']=kwargs[key]['max']-kwargs[key]['min']
    ...
    #     Mix(dictionary=None, total_flow=None, equilibrate=False)
    #
    #
